...
export default function VideoModelUpdateForm(props) {
  const {
    id: idProp,
    videoModel: videoModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = videoModelRecord
      ? { ...initialValues, ...videoModelRecord }
      : initialValues;
    setVideoUrl(cleanValues.videoUrl);
    setVideoDateTime(cleanValues.videoDateTime);
    setVideoTitle(cleanValues.videoTitle);
    setVideoType(cleanValues.videoType);
    setStatus(cleanValues.status);
    setAddDateTime(cleanValues.addDateTime);
    setOtherData(
      typeof cleanValues.otherData === "string"
        ? cleanValues.otherData
        : JSON.stringify(cleanValues.otherData)
    );
    setAddIpAddress(cleanValues.addIpAddress);
    setVideoViews(cleanValues.videoViews);
    setVideoID(cleanValues.videoID);
    setVideoCategory(cleanValues.videoCategory);
    setVideoDescription(cleanValues.videoDescription);
    setRatingNo(cleanValues.ratingNo);
    setRatingCount(cleanValues.ratingCount);
    setCommentCount(cleanValues.commentCount);
    setChannelmodelID(cleanValues.channelmodelID);
    setVideoCategoryId(cleanValues.videoCategoryId);
    setVideoTags(cleanValues.videoTags);
    setVideoLikeCount(cleanValues.videoLikeCount);
    setRatingYes(cleanValues.ratingYes);
    setViralCheckEndDateTime(cleanValues.viralCheckEndDateTime);
    setViralFinalScore(cleanValues.viralFinalScore);
    setViralViewsRquired(cleanValues.viralViewsRquired);
    setSearch(cleanValues.search);
    setVideoClickCount(cleanValues.videoClickCount);
    setErrors({});
  };
  const [videoModelRecord, setVideoModelRecord] =
    React.useState(videoModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(VideoModel, idProp)
        : videoModelModelProp;
      setVideoModelRecord(record);
    };
    queryData();
  }, [idProp, videoModelModelProp]);
  React.useEffect(resetStateValues, [videoModelRecord]);
  const validations = {
    videoUrl: [],
    videoDateTime: [],
    videoTitle: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            VideoModel.copyOf(videoModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "VideoModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Video url"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || videoModelModelProp)}
