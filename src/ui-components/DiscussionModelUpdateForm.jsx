...
export default function DiscussionModelUpdateForm(props) {
  const {
    id: idProp,
    discussionModel: discussionModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = discussionModelRecord
      ? { ...initialValues, ...discussionModelRecord }
      : initialValues;
    setComment(cleanValues.comment);
    setCommentType(cleanValues.commentType);
    setStatus(cleanValues.status);
    setAddDateTime(cleanValues.addDateTime);
    setAddIpAddress(cleanValues.addIpAddress);
    setUserHandle(cleanValues.userHandle);
    setOtherData(
  ...
  const [discussionModelRecord, setDiscussionModelRecord] = React.useState(
    discussionModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(DiscussionModel, idProp)
        : discussionModelModelProp;
      setDiscussionModelRecord(record);
    };
    queryData();
  }, [idProp, discussionModelModelProp]);
  React.useEffect(resetStateValues, [discussionModelRecord]);
  const validations = {
    comment: [],
    commentType: [],
    status: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            DiscussionModel.copyOf(discussionModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DiscussionModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Comment"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || discussionModelModelProp)}
