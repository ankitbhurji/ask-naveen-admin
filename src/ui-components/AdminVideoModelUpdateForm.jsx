...
export default function AdminVideoModelUpdateForm(props) {
  const {
    id: idProp,
    adminVideoModel: adminVideoModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = adminVideoModelRecord
      ? { ...initialValues, ...adminVideoModelRecord }
      : initialValues;
    ...
    setViewsCount(cleanValues.viewsCount);
    setLikesCount(cleanValues.likesCount);
    setVideoCategoryId(cleanValues.videoCategoryId);
    setVideoTags(cleanValues.videoTags);
    setVideoPublishDateTime(cleanValues.videoPublishDateTime);
    setSearch(cleanValues.search);
    setErrors({});
  };
  const [adminVideoModelRecord, setAdminVideoModelRecord] = React.useState(
    adminVideoModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(AdminVideoModel, idProp)
        : adminVideoModelModelProp;
      setAdminVideoModelRecord(record);
    };
    queryData();
  }, [idProp, adminVideoModelModelProp]);
  React.useEffect(resetStateValues, [adminVideoModelRecord]);
  const validations = {
    videoID: [],
    videoUrl: [],
    videoTitle: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            AdminVideoModel.copyOf(adminVideoModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AdminVideoModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Video id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || adminVideoModelModelProp)}
