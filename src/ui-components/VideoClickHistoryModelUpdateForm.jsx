...
export default function VideoClickHistoryModelUpdateForm(props) {
  const {
    id: idProp,
    videoClickHistoryModel: videoClickHistoryModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = videoClickHistoryModelRecord
      ? { ...initialValues, ...videoClickHistoryModelRecord }
      : initialValues;
    setVideoId(cleanValues.videoId);
    setVideoModelID(cleanValues.videoModelID);
    setVideoClickTime(cleanValues.videoClickTime);
    setUserModelID(cleanValues.userModelID);
    setChannelModelID(cleanValues.channelModelID);
    setErrors({});
  };
  const [videoClickHistoryModelRecord, setVideoClickHistoryModelRecord] =
    React.useState(videoClickHistoryModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(VideoClickHistoryModel, idProp)
        : videoClickHistoryModelModelProp;
      setVideoClickHistoryModelRecord(record);
    };
    queryData();
  }, [idProp, videoClickHistoryModelModelProp]);
  React.useEffect(resetStateValues, [videoClickHistoryModelRecord]);
  const validations = {
    videoId: [],
    videoModelID: [],
    videoClickTime: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            VideoClickHistoryModel.copyOf(
              videoClickHistoryModelRecord,
              (updated) => {
      ...
      {...getOverrideProps(overrides, "VideoClickHistoryModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Video id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || videoClickHistoryModelModelProp)}
